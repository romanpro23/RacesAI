...
class GameController:
    ...
    def __init__(self, car: Car, agent: Agent = None, level: int = 1, fps=60, width=1024, height=768, frequency_ai=2):
        self.fps = fps
        self.state = None

        self.generator = Generator(f"maps/map_{level}.txt", width, height)
        self.background = Background(f"maps/background_{level}.png")
        self.environment = Environment(self.generator.frames, self.generator.rewards, self.generator.finish)
